...
-- 4-) Soma dos elementos nas posições pares da lista - versão 1 (usando tam)
somaIndicesPares1 [] = 0
somaIndicesPares1 (x:xs)
  | mod (tam xs) 2 == 0 = x + somaIndicesPares1 xs 
  | otherwise = somaIndicesPares1 xs

-- 4-) Soma dos elementos nas posições pares da lista - versão 2 (sem usar tam)
...
-- 7-) Conta quantas vezes item aparece na lista - versão 1 (com acumuladora)
...
-- 7-) Conta quantas vezes item aparece na lista - versão 2 (sem acumuladora)
